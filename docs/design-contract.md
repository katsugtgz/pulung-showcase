...
> Every section author implementing tickets under issue #50 must read this
> before adding or refreshing a section.

**Status:** Accepted (issue #51, 2026-07-18). Companion to ADR-004.

## 1. Inspiration synthesis

The 2026 landing refresh (issue #50) synthesizes lessons from four product
...
**Section wrapper pattern** (informal but mandatory — see
...
- Use an SVG placeholder generated by the `agy` CLI (per AGENTS.md).
...
photos that imply "dijamin lulus" (guaranteed pass — see AGENTS.md
anti-pattern).
...
Its baseline rule (issue #50 story #24):
...
cleanly at all four — verified via `agent-browser` CLI (Playwright is
forbidden repo-wide).
...
**Don't:**
- Don't reintroduce stitch purple `#5e4399`. The palette is locked.
...
- Don't resolve `// TODO: verify owner` markers in
  `src/lib/copy/data.ts` with invented values.
